...
tickers <- c("XLK","XLY","XLV","XLF","XLI","XLP","XLU","XLB","XLE")

# Download 11 ETFs and SPY

getSymbols(tickers,from="2006-08-22", to="2010-12-29")
getSymbols("SPY",from="2006-08-22",to="2010-12-29")


P <- merge(XLK$XLK.Close,XLY$XLY.Close,XLV$XLV.Close,XLF$XLF.Close,XLI$XLI.Close,
           XLP$XLP.Close,XLU$XLU.Close,XLB$XLB.Close,XLE$XLE.Close)
...
XLRE <- read.xlsx("XLRE.xlsx",1)
XLC <- read.xlsx("XLC.xlsx",1)

Date <- index(SPY[2:1096,1])
XLRE <- xts(XLRE,order.by=Date)
XLRE <- XLRE[,2]
XLC <- xts(XLC,order.by=Date)
XLC <- XLC[,2]
...
Fa <- read.csv("Fama_from060823to101228.csv")
Fa <- Fa[,-1]
...
rownames(Beta) <- c("XLK","XLY","XLV","XLF","XLI","XLP","XLU","XLB","XLE","XLRE","XLC")
Beta

colnames(Coef) <- c("Alpha","RMRF","SMB","HML")
rownames(Coef) <- c("XLK","XLY","XLV","XLF","XLI","XLP","XLU","XLB","XLE","XLRE","XLC")
...
Optimizer <- function(lambda,T_Beta,wp) {
  ...
  if (det(as.matrix(2*lambda*Q)) < 0.1) {
    ...
    rownames(w) <- c("XLK","XLY","XLV","XLF","XLI","XLP","XLU","XLB","XLE","XLRE","XLC")
...
Port_Return1 <- function(startday,endday,LB_Mu,LB_Q,T_Beta,lambda){
  ...
  rownames(wp) <- c("XLK","XLY","XLV","XLF","XLI","XLP","XLU","XLB","XLE","XLRE","XLC") 
  colnames(wp) <- c("Weight")
...
ret1 <- Port_Return1("2007-01-03","2010-12-28",60,90,1,1)
...
ret2 <- Port_Return2("2007-01-03","2010-12-28",60,90,1,1)
...
Port_Return2("2007-01-03","2010-12-28",60,90,1,1)
...
getSymbols("GLD",from="2007-01-03",to="2010-12-28")
getSymbols("TLT",from="2007-01-03",to="2010-12-28")
...
ret3 <- Port_Return3("2007-01-03","2010-12-28",60,90,1,1)
...
ret4 <- Port_Return4("2007-01-03","2010-12-28",60,90,1,1)
...
ret5 <- Port_Return5("2007-01-03","2010-12-28",60,90,1,1)
...
#write.xlsx(K_Data,"z3_subP.xlsx")
#write.xlsx(Table_K,"Table_z3_subP.xlsx")
